...
#include <string> // TODO: unnecessary?
...
TEST_CASE("extract_stats_from_sensor using event27.csv", "[process_data],[static]") {
	...
	const float expected_results[2][2][feats_sensor_count] = { // TODO: detail expected data using comments
		...
		-0.77437142857143, 1.0480571428571, 2.2330142857143, 3.0619488526634, 1.2534808664072, 2.3925670045423, 9.3755307763265, 1.571214282449, 5.7243768712245, -5.432, -0.977, -3.052, 5.493, 3.845, 5.554, -0.6205828634130963, -0.4365929652505005, -0.8253146073298572, 0.4133372071581709, 0.6807733153487078, -0.6037826423274072
	};

	read_data_from_csv(imu_data, "event27.csv");

	std::string info_text_sensor = sensor == ACCEL ? "ACCEL" : "GYROS";
	// TODO: SECTION vs DYNAMIC_SECTION
	DYNAMIC_SECTION(info_text_sensor + " as sensor with first_pos=" + std::to_string(first_pos)) {
		extract_stats_from_sensor(features, sensor, imu_data, first_pos);

		const int i = sensor == ACCEL ? 0 : 1;
		const int j = first_pos == 0 ? 0 : 1;
		const float (&expected)[feats_sensor_count] = expected_results[i][j];
...
TEST_CASE("extract_features using event27.csv", "[process_data],[static]") {
	...
	float features[43];
	std::vector<double> imu_data[7];
	const float expected_results[2][2][21] = { // TODO: detail expected data using comments
		-0.042571428571429, -0.55227142857143, -0.84475714285714, 0.0085916428307173, 0.0091463051534108, 0.0050665167359398, 0.000073816326530616, 0.000083654897959308, 0.000025669591835558, -0.069, -0.580, -0.857, -0.027, -0.537, -0.835, 0.8826789660749395, 0.563486000435029, -0.6289091800652478, -0.8224259442845651, -0.9375635402876765, -0.45971082883459685,
		-0.034742857142857, -0.56345714285714, -0.8379, 0.0086663317570036, 0.011566806837162, 0.0088964679989436, 0.000075105306122449, 0.00013379102040822, 0.000079147142856227, -0.055, -0.589, -0.855, -0.018, -0.532, -0.820, -0.6349873741886602, 0.3321879782556616, -0.994875225964436, -0.1457982203222088, 0.06111678783540695, 0.17019665034162287,
		-1.5973142857143, 0.42201428571429, -0.77087142857143, 0.7256668173452, 0.85610552575281, 0.9823307403667, 0.52659232979592, 0.73291667122449, 0.96497368346939, -2.869, -2.136, -3.052, -0.061, 1.953, 0.854, -0.7490513065291156, 1.0112479704644457, -0.4723143922086641, 0.25161425088606093, -0.828537796196954, -0.4854193239006291,
		-0.77437142857143, 1.0480571428571, 2.2330142857143, 3.0619488526634, 1.2534808664072, 2.3925670045423, 9.3755307763265, 1.571214282449, 5.7243768712245, -5.432, -0.977, -3.052, 5.493, 3.845, 5.554, -0.6205828634130963, -0.4365929652505005, -0.8253146073298572, 0.4133372071581709, 0.6807733153487078, -0.6037826423274072
	...
	SECTION("using first_pos=0") {
		extract_features(features, imu_data, 0);
		// TODO: can be simplified taking adventage of contiguous memory data of arrays
		const float (&expected_accel)[21] = expected_results[0][0];
		const float (&expected_gyros)[21] = expected_results[1][0];

		CHECK(features[0] == Approx(113193.5).margin(me));
		for (int i = 1; i <= 21; i++) {
			CHECK(features[i] == expected_accel[i-1]);
		}
		for (int i = 22; i <= 42; i++) {
			CHECK(features[i] == expected_gyros[i-22]);
		}
	}

	SECTION("using first_pos=59") {
		extract_features(features, imu_data, 59);
		// TODO: can be simplified taking adventage of contiguous memory data of arrays
		const float (&expected_accel)[21] = expected_results[0][1];
		const float (&expected_gyros)[21] = expected_results[1][1];

		CHECK(features[0] == Approx(113252.5).margin(me));
		for (int i = 1; i <= 21; i++) {
			CHECK(features[i] == expected_accel[i-1]);
		}
		for (int i = 22; i <= 42; i++) {
...
TEST_CASE("estimate_time", "[process_data],[static]") {
	...
	// timestamps generated randomly from https://onlinenumbertools.com/generate-random-numbers and sorted with https://onlinenumbertools.com/sort-numbers

	SECTION("windows vector structure: S") {
		std::vector<movement> windows { START };
		std::vector<double> timestamps { 119.5313000 };

		CHECK(estimate_time(windows, timestamps) == Approx(-1.0).margin(me));
	}
	...
	SECTION("windows vector structure: S-F") {
		...
		CHECK(estimate_time(windows, timestamps) == Approx(16.0812752).margin(me));
	}
	SECTION("windows vector structure: S-O-F") {
		...
		CHECK(estimate_time(windows, timestamps) == Approx(49.9415552).margin(me));
	}
	SECTION("windows vector structure: S-S-S-O-O-F") {
		...
		CHECK(estimate_time(windows, timestamps) == Approx(104.67096763).margin(me));
	}
	SECTION("windows vector structure: S-O-O-F-F-F") {
		...
		CHECK(estimate_time(windows, timestamps) == Approx(95.22869813).margin(me));
	}
	SECTION("windows vector structure: S-S-O-O-O-F-F-F") {
		...
		CHECK(estimate_time(windows, timestamps) == Approx(94.72718012).margin(me));
	}
	...
	SECTION("windows vector structure: O-O-S-O-O-O-F-F") {
		...
		CHECK(estimate_time(windows, timestamps) == Approx(101.85486875).margin(me));
	}
	SECTION("windows vector structure: S-S-S-S-O-F-F-O-O-O") {
		...
		CHECK(estimate_time(windows, timestamps) == Approx(73.15742868).margin(me));
	}
	SECTION("windows vector structure: O-S-S-O-O-F-F-F-F-O") {
		...
		CHECK(estimate_time(windows, timestamps) == Approx(95.30084235).margin(me));
	}
}
...
TEST_CASE("process_data: expected <5 percentage of failures", "[process_data]") {
	...
	for (int i = 1; i <= test_file_count; i++) {
		...
		const double expected_duration = read_data_from_csv(imu_data, filename);
		const double estimation = process_data(imu_data);

		if (estimation == Approx(expected_duration).margin(160.0)) { // TODO: margin 160.0 - low demand
			success_count++;
		} else {
			WARN("failed estimation of file " << filename << ": " << estimation << " == " << expected_duration);
			fail_count++;
		}
	}
